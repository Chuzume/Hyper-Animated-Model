...
local model_file_name = "model"
models[model_file_name]:setPos(20,24,0)
...
-- Ping(Left Click)
if player.isUsingItem() == false and attackKey.isPressed() == true and interactKey.isPressed() == false and dropKey.isPressed() == false then
-- パンチ
...
-- 立ち泳ぎ
    ...
    if string.find(Mainhand,"air") then
        ...
        pose.armRight = {math.cos(time*0.2)*20-20,0,math.sin(time*0.2)*-5+10}
...
if string.find(Mainhand,"air") then
    ...
    model.Body.RightArm.RightHand.RIGHT_HELD_ITEM.setRot({30,0})
    model.Body.RightArm.RightHand.RIGHT_HELD_ITEM.setPos({0,0,-2})
end
...
if HandMoved == false then
    if ComboUse < 20 then--retracting arm
        ...
        pose.body[2] = pose.body[2]+ 45
        pose.head[2] = pose.head[2]- 45
        model.Body.RightArm.RightHand.RIGHT_HELD_ITEM.setRot({-25,180})
        model.Body.RightArm.RightHand.RIGHT_HELD_ITEM.setPos({0,-1,-1})
    end
